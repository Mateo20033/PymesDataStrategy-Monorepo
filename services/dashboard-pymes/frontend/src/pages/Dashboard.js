...
import { getDatasets, eliminarDataset, limpiarDatasets } from '../services/uploadService';
import { enviarPregunta } from '../services/chatService';
import { exportarPDF } from '../utils/pdfExport';
import AnalisisAvanzadoModal from '../components/AnalisisAvanzadoModal';
...
const ChartCard = ({ grafico, tipo, analisis, onAnalizar }) => (
  ...
  >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            background: analisis?.texto ? '#4f46e5' : '#eef2ff',
            color: analisis?.texto ? '#fff' : '#4f46e5',
        ...
        >
          {analisis?.loading ? (
            <>
              <span className="_spin" style={{ display: 'inline-block', width: 10, height: 10, border: '2px solid #4f46e5', borderTopColor: 'transparent', borderRadius: '50%' }} />
              Analizando…
            </>
          ) : (
            <>
    ...
    {analisis?.texto && (
      <div style={{
        borderTop: '1px solid #e0e7ff', background: '#f5f7ff',
        padding: '10px 14px', borderRadius: '0 0 10px 10px',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px' }}>
          <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="#4f46e5" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
          </svg>
          <span style={{ fontSize: '11px', fontWeight: 700, color: '#4f46e5' }}>Análisis IA</span>
        </div>
  ...
  return (
    <div style={{
      ...
      <Link to={linkTo} style={{
        padding: '8px 18px', borderRadius: '8px', background: '#4f46e5',
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px 20px', marginBottom: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#4f46e5" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        <span style={{ fontSize: '13px', fontWeight: 700, color: '#111827' }}>Insights detectados automáticamente</span>
        <span style={{ background: '#eef2ff', color: '#4f46e5', borderRadius: '10px', padding: '1px 7px', fontSize: '11px', fontWeight: 700 }}>{insights.length}</span>
      </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    getDatasets().then(res => {
      const lista = res.data.data.datasets || [];
      setDatasets(lista);
  ...
  const analizarGrafico = useCallback(async (grafico) => {
    ...
    try {
      ...
      setChartAnalysis(prev => ({
        ...prev,
        [grafico.id]: { loading: false, texto: res.data.respuesta, error: null },
      }));
    } catch {
      setChartAnalysis(prev => ({
        ...prev,
    ...
    <div className="page">
      ...
      <div className="page__header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          {!sinDatos && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 12px', borderRadius: '20px', background: '#eef2ff', color: '#4f46e5', fontSize: '12px', fontWeight: 600 }}>
              <IconoTipo tipo={tipo} />
          ...
          {!sinDatos && (
            <button
              ...
              style={{
                ...
                background: exportandoPDF ? '#6b7280' : '#4f46e5',
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: activo ? '#4f46e5' : '#f1f5f9',
                  border: `1.5px solid ${activo ? '#4f46e5' : '#e2e8f0'}`,
        ...
        {FILTROS.map(f => {
          ...
          return (
            ...
            >
              ...
              {cnt > 0 && (
                <span style={{
                  background: filtro === f.id ? '#4f46e5' : '#e5e7eb',
      ...
      {/* Banner de estado */}
      <div className={`info-banner ${sinDatos ? 'info-banner--warn' : ''}`} style={{ marginTop: '8px' }}>
        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        {loading ? 'Analizando datos...'
          : sinDatos
          ? 'No tienes datos cargados. Ve a "Cargar CSV" para importar tu primer archivo.'
          : `${TIPO_LABELS[tipo] ?? tipo} · ${s.total_datasets} dataset(s) · ${s.total_registros?.toLocaleString()} registros`
            + (s.col_fecha     ? ` · fecha: "${s.col_fecha}"`         : ' · sin columna de fecha')
            + (s.col_categoria ? ` · categoría: "${s.col_categoria}"` : ' · sin columna de categoría')
            + ` · ${graficos.length} gráfico(s) generado(s)`
        }
      </div>
      ...
      <div style={{ marginTop: '20px', background: 'linear-gradient(135deg,#1a1a1a 0%,#3d1a00 60%,#ff6600 100%)', borderRadius: '14px', padding: '24px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
          {sinDatos && (
            <Link to="/upload" style={{ padding: '10px 20px', borderRadius: '9px', background: 'rgba(255,255,255,0.15)', color: '#fff', fontSize: '13px', fontWeight: 600, border: '1px solid rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <Link to="/chatbot" style={{ padding: '10px 22px', borderRadius: '9px', background: '#ff6600', color: '#fff', fontSize: '13px', fontWeight: 700, border: 'none', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 14px rgba(255,102,0,0.45)' }}>
